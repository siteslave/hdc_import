...
BACKUP_DIR = parser.get('import', 'backup')
# Zip files list
...
def do_extract(zip_file):
    ...
    try:
        ...
        dst_file = os.path.join(BACKUP_DIR, zip_file_name)
        ...
        utils.log("Error: %s - %s \r" % (zip_file, e.strerror))
...
def get_file_contents(text_file):
    ...
    try:
        ...
        utils.log('[FILE_CONTENT] Error: %s \r' % e)
    ...
    if zip_count:
        ...
        try:
            ...
            for f in zip_files:
                ...
                utils.log('[IMPORT] %s success.' % zip_file_name)
        ...
        utils.log('CID MAPPING [%s]...' % rec_id)
        print('CID Mapping...')
